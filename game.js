document.addEventListener("DOMContentLoaded", function () {
  ...
  function startGame() {
      ...
      for (let i = 0; i < cities.length; i++) {
          ...
          ball.style.setProperty('--ball-index', i); // 각 공에 인덱스 값 설정
          ball.textContent = cities[i]; // 도시 이름을 공에 넣음
          ballsContainer.appendChild(ball);
      }
  }

  // 뽑기 버튼 클릭 시 랜덤으로 도시 선택
  drawButton.addEventListener("click", function () {
      const selectedCountry = countrySelect.value;
      const cities = countries[selectedCountry];

      const randomCity = cities[Math.floor(Math.random() * cities.length)];
      resultText.textContent = `이번 여행지는 ${randomCity} 입니다!`;
  ...
  playAgainButton.addEventListener("click", function () {
      resultText.textContent = '이번 여행지는 ??? 입니다!';
      playAgainButton.style.display = "none";
      countrySelection.style.display = "block";
      gameScreen.style.display = "none";
      countrySelect.value = "";  // 국가 선택 초기화
      startButton.disabled = true;
  });

  // 종료하기 버튼 클릭 시 페이지 닫기 (혹은 다른 페이지로 이동)
  exitButton.addEventListener("click", function () {
      window.close(); // 페이지 종료
      // 또는 window.location.href = "다른페이지.html"; 로 다른 페이지로 이동 가능
  });
});
